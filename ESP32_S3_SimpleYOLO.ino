...
void convertRGB565toRGB888() {
  Serial.println("create image buff");
  rgb565Data = Get_Image_frameBuffer();
...
void setup() {
  ...
  tft.begin();
  tft.setRotation(3);  // 0-3: xoay màn (1 = ngang WIDTHxHEIGHT)
...
void loop() {
  long tick_total = millis();
  long tick_get_image = millis();
  // fetchAndSaveImage888(inputImage);
  capture_BmpImage_qqvga();
  Serial.print("time get_image: ");
  Serial.println(millis() - tick_get_image);

  long tick_convert_image = millis();
  convertRGB565toRGB888();
  Serial.print("time convert_image: ");
  Serial.println(millis() - tick_convert_image);

  long tick_feedforward = millis();
  CNN_FeedForward(inputImage, &head_conv[0][0][0]);
  Serial.print("time feedforward: ");
  Serial.println(millis() - tick_feedforward);

  Serial.println("Draw image+box");
  long tick_Draw = millis();
  ...
  for (int i = 0; i < h_head; i++) {
    for (int j = 0; j < w_head; j++) {
      ...
      if (conf > CONF_THRESHOLD) {
        ...
        // drawPredictionBox(outputImage, x1, y1, x2, y2, label[index], conf_to_string, output_image_name);

        tft.drawRect(draw_offset_x + x1 * WIDTH,draw_offset_x + y1 * HEIGHT, w * WIDTH, h * HEIGHT, ILI9341_RED);
        tft.setTextSize(1);
        tft.setCursor(draw_offset_x + x1 * WIDTH, draw_offset_x + y1 * HEIGHT);
        tft.print(label[index]);
        tft.print(conf_to_string);
      }
    }
  }
  Serial.print("time Draw image+box: ");
  Serial.println(millis() - tick_Draw);
  Serial.print("time total process: ");
  Serial.println(millis() - tick_total);
